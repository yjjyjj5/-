...
namespace 俄罗斯方块
{
    public partial class FrmMain : Form
    {
        ...
        public void 画当前方块()
        {
            ...
            //画残影
            for (int index = 0; index < 残影集合.Count; index++)
            {
                残影信息 cy = 残影集合[index];
                ...
                for (int i = 0; i < 4; i++)
                {
                    for (int j = 0; j < 4; j++)
                    {
                        if (cy.Dzt[i, j] == 1)
                            g.FillRectangle(
                                new SolidBrush(Color.FromArgb(100 - (index * 40), 0, 255, 0))
                                , (j * 25) + (cy.Col * 25), (i * 25) + (cy.Row * 25), 24, 24);
                    }
                }
            }
        ...
        List<残影信息> 残影集合 = new List<残影信息>();
        private void 产生残影()
        {
            if (fk == null) return;

            残影信息 cy = new 残影信息(fk.Row, fk.Col, fk.GetCurrentDzt());
            残影集合.Insert(0, cy);
            if (残影集合.Count > 3)
                残影集合.RemoveAt(残影集合.Count - 1);
        }
    }
}
